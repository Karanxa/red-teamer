"""
Benchmark engine for evaluating LLM performance against attack vectors.
"""
...
class BenchmarkEngine:
    """
    Engine for benchmarking LLM security against attack vectors.
    
    This class handles the benchmarking process, including:
    1. Loading models and attack vectors
    2. Running benchmarks with various parameters
    ...
    def run_benchmark(self) -> Dict:
        ...
        self.logger.info(f"Starting benchmark with {total_vectors} vectors against {total_models} models")
    ...
    def _save_results(self, results: List[Dict], summary: Dict) -> None:
        ...
        # Create filename based on benchmark name and timestamp
        timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
        benchmark_name = self.config['name'].replace(' ', '_').lower()
        filename = f"{benchmark_name}_{timestamp}"
